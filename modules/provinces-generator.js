  ...
  const generate = (regenerate = false, regenerateLockedStates = false) => {
    ...
    states.forEach(s => {
      ...
      for (let i = 0; i < provincesNumber; i++) {
        ...
        const color = getMixedColorUnique(s.color,0.2,0.3,provinces);// need a util function to make sure it's unique.// could pass provinces here.  
    ...
    states.forEach(s => {
      ...
      while (stateNoProvince.length) {
        ...
        const color = getMixedColorUnique(s.color,0.2,0.3,provinces);
